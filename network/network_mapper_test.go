		...
		BeforeEach(func() {
			...
			networkPayload = models.NetworkPayload{
				Properties: models.Properties{
					...
					SpaceGUID: fmt.Sprintf("some-space-%x", rand.Int()),
				},
			}
		})

		It("sets the network ID equal to the space GUID", func() {
			networkID, err := networkMapper.GetNetworkID(networkPayload)
			Expect(err).NotTo(HaveOccurred())
			Expect(networkID).To(Equal(networkPayload.Properties.SpaceGUID))
		})

		Context("when the space ID is not set", func() {
			BeforeEach(func() {
				networkPayload.Properties.SpaceGUID = ""
			})
